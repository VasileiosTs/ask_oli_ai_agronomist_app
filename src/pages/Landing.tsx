...
const STATS = (lang: string) => {
  const labels: Record<string, [string, string, string, string, string]> = {
    el: ['Ασθένειες & παθογόνα', 'Φυτά & καλλιέργειες', 'Ελλείψεις θρεπτικών', 'Γλώσσες', 'Τεχνικοί υπολογισμοί'],
    en: ['Diseases & pathogens', 'Plants & crops', 'Nutrient deficiencies', 'Languages', 'Technical calculations'],
    it: ['Malattie e patogeni', 'Piante e colture', 'Carenze nutrizionali', 'Lingue', 'Calcoli tecnici'],
    es: ['Enfermedades y patógenos', 'Plantas y cultivos', 'Deficiencias nutricionales', 'Idiomas', 'Cálculos técnicos'],
    fr: ['Maladies et pathogènes', 'Plantes et cultures', 'Carences nutritionnelles', 'Langues', 'Calculs techniques'],
    ar: ['الأمراض والمسببات', 'نباتات ومحاصيل', 'نقص العناصر الغذائية', 'اللغات', 'حسابات تقنية'],
    tr: ['Hastalıklar & patojenler', 'Bitki ve ürünler', 'Besin eksiklikleri', 'Dil', 'Teknik hesaplamalar'],
    ro: ['Boli și agenți patogeni', 'Plante și culturi', 'Deficiențe nutriționale', 'Limbi', 'Calcule tehnice'],
    bg: ['Болести и патогени', 'Растения и култури', 'Хранителни дефицити', 'Езика', 'Технически изчисления'],
    sq: ['Sëmundje & patogjenë', 'Bimë e kultura', 'Mungesa ushqyese', 'Gjuhë', 'Llogaritje teknike'],
    pt: ['Doenças e patógenos', 'Plantas e culturas', 'Deficiências nutricionais', 'Idiomas', 'Cálculos técnicos'],
    de: ['Krankheiten & Erreger', 'Pflanzen & Kulturen', 'Nährstoffmängel', 'Sprachen', 'Technische Berechnungen'],
    hi: ['रोग और रोगजनक', 'पौधे और फसलें', 'पोषण की कमी', 'भाषाएं', 'तकनीकी गणनाएं'],
    sw: ['Magonjwa na vimelea', 'Mimea na mazao', 'Upungufu wa virutubisho', 'Lugha', 'Hesabu za kiufundi'],
    bn: ['রোগ ও রোগজীবাণু', 'গাছ ও ফসল', 'পুষ্টির ঘাটতি', 'ভাষা', 'প্রযুক্তিগত গণনা'],
    id: ['Penyakit & patogen', 'Tumbuhan & tanaman', 'Defisiensi nutrisi', 'Bahasa', 'Kalkulasi teknis'],
    am: ['ሕመሞች እና ፓቶጀኖች', 'ተክሎች እና ሰብሎች', 'የንጥረ-ነገር እጥረቶች', 'ቋንቋዎች', 'ቴክኒካዊ ስሌቶች'],
    vi: ['Bệnh & mầm bệnh', 'Cây cối & cây trồng', 'Thiếu dinh dưỡng', 'Ngôn ngữ', 'Tính toán kỹ thuật'],
    ha: ['Cututtuka & ƙwayoyin cuta', 'Tsire-tsire da amfanin gona', 'Ƙarancin abinci', 'Harsuna', 'Ƙididdiga na fasaha'],
    ur: ['بیماریاں اور جراثیم', 'پودے اور فصلیں', 'غذائی کمیاں', 'زبانیں', 'تکنیکی حسابات'],
  };
...
// ── Breadth strip ─────────────────────────────────────────────────────────────
// One band that shows Oli's range, expert cases first (signals depth, keeps us
// out of the casual "plant ID app" bucket), everyday cases last.
type BreadthCard = { emoji: string; label: string; q: string };
type BreadthT = { heading: string; sub: string; cards: BreadthCard[] };
const BREADTH = (lang: string): BreadthT => {
  const b: Record<string, BreadthT> = {
    en: {
      heading: 'One expert for everything you grow.',
      sub: 'Exact diagnosis, precise dosage, and follow-up. From a commercial grove to a windowsill pot.',
      cards: [
        { emoji: '🌾', label: 'Field crops',     q: 'Early blight across 2 hectares of tomatoes. Treatment plan?' },
        { emoji: '🍒', label: 'Orchards',        q: 'Brown rot on my cherry trees. Exact organic and chemical dose?' },
        { emoji: '🔬', label: 'Agronomists',     q: "Calculate my orange grove's July water need (ETc), Kc 0.85." },
        { emoji: '🌹', label: 'Gardens',         q: 'Black spot on my roses. What treats it, and when?' },
        { emoji: '🌱', label: 'Lawns',           q: 'Brown patches spreading across my lawn. What is it?' },
        { emoji: '🪴', label: 'Home & balcony',  q: 'White spots on my basil. What should I do?' },
      ],
    },
    el: {
      heading: 'Ένας ειδικός για ό,τι καλλιεργείς.',
      sub: 'Ακριβής διάγνωση, σωστή δόση και παρακολούθηση. Από εμπορικό περιβόλι μέχρι μια γλάστρα στο παράθυρο.',
      cards: [
        { emoji: '🌾', label: 'Καλλιέργειες',      q: 'Αλτερνάρια σε 2 εκτάρια ντομάτες. Πλάνο θεραπείας;' },
        { emoji: '🍒', label: 'Οπωρώνες',          q: 'Φαιά σήψη στις κερασιές μου. Ακριβής βιολογική και χημική δόση;' },
        { emoji: '🔬', label: 'Γεωπόνοι',          q: 'Υπολόγισε την ανάγκη σε νερό (ETc) για τον πορτοκαλεώνα μου τον Ιούλιο, Kc 0.85.' },
        { emoji: '🌹', label: 'Κήποι',             q: 'Μαύρη κηλίδα στις τριανταφυλλιές μου. Τι τη θεραπεύει και πότε;' },
        { emoji: '🌱', label: 'Γκαζόν',            q: 'Καφέ μπαλώματα απλώνονται στο γκαζόν μου. Τι είναι;' },
        { emoji: '🪴', label: 'Σπίτι & μπαλκόνι',  q: 'Άσπρες κηλίδες στον βασιλικό μου. Τι να κάνω;' },
      ],
    },
    it: {
      heading: 'Un esperto per tutto ciò che coltivi.',
      sub: 'Diagnosi esatta, dosaggio preciso e follow-up. Da un frutteto commerciale a un vaso sul davanzale.',
      cards: [
        { emoji: '🌾', label: 'Colture di campo', q: 'Alternariosi su 2 ettari di pomodori. Piano di trattamento?' },
        { emoji: '🍒', label: 'Frutteti', q: 'Monilia sui miei ciliegi. Dose esatta biologica e chimica?' },
        { emoji: '🔬', label: 'Agronomi', q: 'Calcola il fabbisogno idrico (ETc) del mio aranceto a luglio, Kc 0.85.' },
        { emoji: '🌹', label: 'Giardini', q: 'Macchia nera sulle mie rose. Cosa la cura e quando?' },
        { emoji: '🌱', label: 'Prati', q: 'Chiazze marroni si allargano sul mio prato. Cosa sono?' },
        { emoji: '🪴', label: 'Casa e balcone', q: 'Macchie bianche sul mio basilico. Cosa devo fare?' },
      ],
    },
    es: {
      heading: 'Un experto para todo lo que cultivas.',
      sub: 'Diagnóstico exacto, dosis precisa y seguimiento. Desde un huerto comercial hasta una maceta en la ventana.',
      cards: [
        { emoji: '🌾', label: 'Cultivos de campo', q: 'Tizón temprano en 2 hectáreas de tomates. ¿Plan de tratamiento?' },
        { emoji: '🍒', label: 'Frutales', q: 'Monilia en mis cerezos. ¿Dosis exacta orgánica y química?' },
        { emoji: '🔬', label: 'Agrónomos', q: 'Calcula la necesidad de agua (ETc) de mi naranjal en julio, Kc 0.85.' },
        { emoji: '🌹', label: 'Jardines', q: 'Mancha negra en mis rosas. ¿Qué la trata y cuándo?' },
        { emoji: '🌱', label: 'Césped', q: 'Manchas marrones se extienden por mi césped. ¿Qué es?' },
        { emoji: '🪴', label: 'Casa y balcón', q: 'Manchas blancas en mi albahaca. ¿Qué hago?' },
      ],
    },
    fr: {
      heading: 'Un seul expert pour tout ce que tu cultives.',
      sub: 'Diagnostic exact, dosage précis et suivi. D\'un verger commercial à un pot sur le rebord de la fenêtre.',
      cards: [
        { emoji: '🌾', label: 'Grandes cultures', q: 'Alternariose sur 2 hectares de tomates. Plan de traitement ?' },
        { emoji: '🍒', label: 'Vergers', q: 'Moniliose sur mes cerisiers. Dose exacte bio et chimique ?' },
        { emoji: '🔬', label: 'Agronomes', q: 'Calcule le besoin en eau (ETc) de mon orangeraie en juillet, Kc 0.85.' },
        { emoji: '🌹', label: 'Jardins', q: 'Taches noires sur mes rosiers. Quel traitement, et quand ?' },
        { emoji: '🌱', label: 'Pelouses', q: 'Des taches brunes s\'étendent sur ma pelouse. Qu\'est-ce que c\'est ?' },
        { emoji: '🪴', label: 'Maison et balcon', q: 'Taches blanches sur mon basilic. Que dois-je faire ?' },
      ],
    },
    ar: {
      heading: 'خبير واحد لكل ما تزرعه.',
      sub: 'تشخيص دقيق، جرعة محددة، ومتابعة. من بستان تجاري إلى أصيص على حافة النافذة.',
      cards: [
        { emoji: '🌾', label: 'المحاصيل الحقلية', q: 'اللفحة المبكرة على هكتارين من الطماطم. خطة العلاج؟' },
        { emoji: '🍒', label: 'البساتين', q: 'العفن البني على أشجار الكرز. الجرعة الدقيقة العضوية والكيميائية؟' },
        { emoji: '🔬', label: 'المهندسون الزراعيون', q: 'احسب حاجة بستان البرتقال للماء (ETc) في يوليو، Kc 0.85.' },
        { emoji: '🌹', label: 'الحدائق', q: 'البقعة السوداء على وردي. ما الذي يعالجها ومتى؟' },
        { emoji: '🌱', label: 'المروج', q: 'بقع بنية تنتشر في مرجي. ما هي؟' },
        { emoji: '🪴', label: 'المنزل والشرفة', q: 'بقع بيضاء على الريحان. ماذا أفعل؟' },
      ],
    },
    tr: {
      heading: 'Yetiştirdiğin her şey için tek uzman.',
      sub: 'Kesin teşhis, net doz ve takip. Ticari bir bahçeden pencere kenarındaki saksıya kadar.',
      cards: [
        { emoji: '🌾', label: 'Tarla bitkileri', q: '2 hektar domateste erken yanıklık. Tedavi planı?' },
        { emoji: '🍒', label: 'Meyve bahçeleri', q: 'Kiraz ağaçlarımda kahverengi çürüklük. Tam organik ve kimyasal doz?' },
        { emoji: '🔬', label: 'Ziraat mühendisleri', q: 'Temmuzda portakal bahçemin su ihtiyacını (ETc) hesapla, Kc 0.85.' },
        { emoji: '🌹', label: 'Bahçeler', q: 'Güllerimde siyah leke. Ne tedavi eder, ne zaman?' },
        { emoji: '🌱', label: 'Çimler', q: 'Çimimde kahverengi lekeler yayılıyor. Bu ne?' },
        { emoji: '🪴', label: 'Ev ve balkon', q: 'Fesleğenimde beyaz lekeler. Ne yapmalıyım?' },
      ],
    },
    ro: {
      heading: 'Un singur expert pentru tot ce cultivi.',
      sub: 'Diagnostic exact, doză precisă și urmărire. De la o livadă comercială la un ghiveci pe pervaz.',
      cards: [
        { emoji: '🌾', label: 'Culturi de câmp', q: 'Alternarioză pe 2 hectare de roșii. Plan de tratament?' },
        { emoji: '🍒', label: 'Livezi', q: 'Putregai brun pe cireșii mei. Doza exactă organică și chimică?' },
        { emoji: '🔬', label: 'Agronomi', q: 'Calculează necesarul de apă (ETc) al livezii mele de portocali în iulie, Kc 0.85.' },
        { emoji: '🌹', label: 'Grădini', q: 'Pată neagră pe trandafirii mei. Ce o tratează și când?' },
        { emoji: '🌱', label: 'Gazon', q: 'Pete maro se întind pe gazonul meu. Ce sunt?' },
        { emoji: '🪴', label: 'Casă și balcon', q: 'Pete albe pe busuiocul meu. Ce să fac?' },
      ],
    },
    bg: {
      heading: 'Един експерт за всичко, което отглеждаш.',
      sub: 'Точна диагноза, прецизна доза и проследяване. От търговска градина до саксия на перваза.',
      cards: [
        { emoji: '🌾', label: 'Полски култури', q: 'Ранен мани върху 2 хектара домати. План за лечение?' },
        { emoji: '🍒', label: 'Овощни градини', q: 'Кафяво гниене по черешите ми. Точна биологична и химична доза?' },
        { emoji: '🔬', label: 'Агрономи', q: 'Изчисли нуждата от вода (ETc) на портокаловата ми градина през юли, Kc 0.85.' },
        { emoji: '🌹', label: 'Градини', q: 'Черни петна по розите ми. Какво ги лекува и кога?' },
        { emoji: '🌱', label: 'Морави', q: 'Кафяви петна се разширяват по моравата ми. Какво е това?' },
        { emoji: '🪴', label: 'Дом и балкон', q: 'Бели петна по босилека ми. Какво да правя?' },
      ],
    },
    sq: {
      heading: 'Një ekspert për gjithçka që kultivon.',
      sub: 'Diagnozë e saktë, dozë e përpiktë dhe ndjekje. Nga një kopsht tregtar te një vazo në dritare.',
      cards: [
        { emoji: '🌾', label: 'Kultura arash', q: 'Alternaria në 2 hektarë domate. Plan trajtimi?' },
        { emoji: '🍒', label: 'Pemishte', q: 'Kalbëzim i murrmë te qershitë e mia. Dozë e saktë organike dhe kimike?' },
        { emoji: '🔬', label: 'Agronomë', q: 'Llogarit nevojën për ujë (ETc) të portokallishtes sime në korrik, Kc 0.85.' },
        { emoji: '🌹', label: 'Kopshte', q: 'Njollë e zezë te trëndafilat e mi. Çfarë e trajton dhe kur?' },
        { emoji: '🌱', label: 'Lëndina', q: 'Njolla kafe po përhapen në lëndinën time. Çfarë janë?' },
        { emoji: '🪴', label: 'Shtëpi dhe ballkon', q: 'Njolla të bardha te borziloku im. Çfarë të bëj?' },
      ],
    },
    pt: {
      heading: 'Um especialista para tudo o que cultivas.',
      sub: 'Diagnóstico exato, dose precisa e acompanhamento. De um pomar comercial a um vaso no parapeito.',
      cards: [
        { emoji: '🌾', label: 'Culturas de campo', q: 'Alternariose em 2 hectares de tomate. Plano de tratamento?' },
        { emoji: '🍒', label: 'Pomares', q: 'Podridão parda nas minhas cerejeiras. Dose exata orgânica e química?' },
        { emoji: '🔬', label: 'Agrónomos', q: 'Calcula a necessidade de água (ETc) do meu laranjal em julho, Kc 0.85.' },
        { emoji: '🌹', label: 'Jardins', q: 'Mancha negra nas minhas roseiras. O que a trata e quando?' },
        { emoji: '🌱', label: 'Relvados', q: 'Manchas castanhas a espalhar-se no meu relvado. O que é?' },
        { emoji: '🪴', label: 'Casa e varanda', q: 'Manchas brancas no meu manjericão. O que faço?' },
      ],
    },
    de: {
      heading: 'Ein Experte für alles, was du anbaust.',
      sub: 'Genaue Diagnose, präzise Dosierung und Nachverfolgung. Vom kommerziellen Obstgarten bis zum Topf auf der Fensterbank.',
      cards: [
        { emoji: '🌾', label: 'Feldkulturen', q: 'Dürrfleckenkrankheit auf 2 Hektar Tomaten. Behandlungsplan?' },
        { emoji: '🍒', label: 'Obstgärten', q: 'Braunfäule an meinen Kirschbäumen. Genaue biologische und chemische Dosis?' },
        { emoji: '🔬', label: 'Agronomen', q: 'Berechne den Wasserbedarf (ETc) meiner Orangenplantage im Juli, Kc 0.85.' },
        { emoji: '🌹', label: 'Gärten', q: 'Schwarzfleckenkrankheit an meinen Rosen. Was hilft, und wann?' },
        { emoji: '🌱', label: 'Rasen', q: 'Braune Flecken breiten sich auf meinem Rasen aus. Was ist das?' },
        { emoji: '🪴', label: 'Zuhause und Balkon', q: 'Weiße Flecken an meinem Basilikum. Was soll ich tun?' },
      ],
    },
    hi: {
      heading: 'आप जो भी उगाते हैं, उस सबके लिए एक विशेषज्ञ।',
      sub: 'सटीक पहचान, सही मात्रा और फ़ॉलो-अप। व्यावसायिक बाग से लेकर खिड़की पर रखे गमले तक।',
      cards: [
        { emoji: '🌾', label: 'खेत की फ़सलें', q: '2 हेक्टेयर टमाटर में अगेती झुलसा। उपचार योजना?' },
        { emoji: '🍒', label: 'फलों के बाग', q: 'मेरे चेरी के पेड़ों पर भूरा सड़न। सटीक जैविक और रासायनिक मात्रा?' },
        { emoji: '🔬', label: 'कृषि विशेषज्ञ', q: 'जुलाई में मेरे संतरे के बाग की पानी की ज़रूरत (ETc) निकालें, Kc 0.85।' },
        { emoji: '🌹', label: 'बग़ीचे', q: 'मेरे गुलाबों पर काला धब्बा। इसका इलाज क्या है और कब?' },
        { emoji: '🌱', label: 'लॉन', q: 'मेरे लॉन पर भूरे धब्बे फैल रहे हैं। यह क्या है?' },
        { emoji: '🪴', label: 'घर और बालकनी', q: 'मेरी तुलसी पर सफ़ेद धब्बे। मुझे क्या करना चाहिए?' },
      ],
    },
    sw: {
      heading: 'Mtaalam mmoja kwa kila kitu unachokuza.',
      sub: 'Uchunguzi sahihi, kipimo kamili, na ufuatiliaji. Kutoka shamba la kibiashara hadi chungu dirishani.',
      cards: [
        { emoji: '🌾', label: 'Mazao ya shambani', q: 'Baka la mapema kwenye hekta 2 za nyanya. Mpango wa tiba?' },
        { emoji: '🍒', label: 'Mabustani ya matunda', q: 'Kuoza kahawia kwenye micheri yangu. Kipimo kamili cha kikaboni na kemikali?' },
        { emoji: '🔬', label: 'Wataalam wa kilimo', q: 'Kokotoa mahitaji ya maji (ETc) ya shamba langu la machungwa Julai, Kc 0.85.' },
        { emoji: '🌹', label: 'Bustani', q: 'Doa jeusi kwenye waridi zangu. Nini huliponya na lini?' },
        { emoji: '🌱', label: 'Nyasi', q: 'Madoa ya kahawia yanaenea kwenye nyasi zangu. Ni nini?' },
        { emoji: '🪴', label: 'Nyumbani na balconi', q: 'Madoa meupe kwenye mrihani wangu. Nifanye nini?' },
      ],
    },
    bn: {
      heading: 'আপনি যা-ই চাষ করেন, সবকিছুর জন্য একজন বিশেষজ্ঞ।',
      sub: 'সঠিক রোগনির্ণয়, নির্দিষ্ট মাত্রা ও ফলো-আপ। বাণিজ্যিক বাগান থেকে জানালার টবে রাখা গাছ পর্যন্ত।',
      cards: [
        { emoji: '🌾', label: 'মাঠের ফসল', q: '2 হেক্টর টমেটোতে আগাম ধসা। চিকিৎসার পরিকল্পনা?' },
        { emoji: '🍒', label: 'ফলের বাগান', q: 'আমার চেরি গাছে বাদামি পচা। সঠিক জৈব ও রাসায়নিক মাত্রা?' },
        { emoji: '🔬', label: 'কৃষিবিদ', q: 'জুলাইয়ে আমার কমলা বাগানের পানির চাহিদা (ETc) হিসাব করুন, Kc 0.85।' },
        { emoji: '🌹', label: 'বাগান', q: 'আমার গোলাপে কালো দাগ। কী দিয়ে সারে আর কখন?' },
        { emoji: '🌱', label: 'লন', q: 'আমার লনে বাদামি দাগ ছড়াচ্ছে। এটা কী?' },
        { emoji: '🪴', label: 'ঘর ও বারান্দা', q: 'আমার তুলসীতে সাদা দাগ। কী করব?' },
      ],
    },
    id: {
      heading: 'Satu ahli untuk semua yang Anda tanam.',
      sub: 'Diagnosis tepat, dosis presisi, dan tindak lanjut. Dari kebun komersial hingga pot di jendela.',
      cards: [
        { emoji: '🌾', label: 'Tanaman pangan', q: 'Hawar dini pada 2 hektar tomat. Rencana perawatan?' },
        { emoji: '🍒', label: 'Kebun buah', q: 'Busuk cokelat pada pohon ceri saya. Dosis organik dan kimia yang tepat?' },
        { emoji: '🔬', label: 'Ahli pertanian', q: 'Hitung kebutuhan air (ETc) kebun jeruk saya di Juli, Kc 0.85.' },
        { emoji: '🌹', label: 'Taman', q: 'Bercak hitam pada mawar saya. Apa obatnya dan kapan?' },
        { emoji: '🌱', label: 'Rumput', q: 'Bercak cokelat menyebar di rumput saya. Apa ini?' },
        { emoji: '🪴', label: 'Rumah dan balkon', q: 'Bercak putih pada kemangi saya. Apa yang harus saya lakukan?' },
      ],
    },
    am: {
      heading: 'ለሚያበቅሉት ሁሉ አንድ ባለሙያ።',
      sub: 'ትክክለኛ ምርመራ፣ ትክክለኛ መጠን እና ክትትል። ከንግድ አትክልት ስፍራ እስከ መስኮት ላይ ድስት።',
      cards: [
        { emoji: '🌾', label: 'የእርሻ ሰብሎች', q: 'በ2 ሄክታር ቲማቲም ላይ ቀደምት ቃጠሎ። የሕክምና ዕቅድ?' },
        { emoji: '🍒', label: 'የፍራፍሬ አትክልቶች', q: 'በቼሪ ዛፎቼ ላይ ቡናማ መበስበስ። ትክክለኛ ኦርጋኒክና ኬሚካል መጠን?' },
        { emoji: '🔬', label: 'የግብርና ባለሙያዎች', q: 'በሐምሌ የብርቱካን አትክልቴን የውሃ ፍላጎት (ETc) አስላ፣ Kc 0.85።' },
        { emoji: '🌹', label: 'የአትክልት ስፍራዎች', q: 'በጽጌረዳዎቼ ላይ ጥቁር ነጥብ። ምን ይፈውሰዋል መቼስ?' },
        { emoji: '🌱', label: 'ሣር', q: 'በሣሬ ላይ ቡናማ ነጥቦች እየተስፋፉ ነው። ምንድን ነው?' },
        { emoji: '🪴', label: 'ቤትና በረንዳ', q: 'በባሲልዬ ላይ ነጭ ነጥቦች። ምን ላድርግ?' },
      ],
    },
    vi: {
      heading: 'Một chuyên gia cho mọi thứ bạn trồng.',
      sub: 'Chẩn đoán chính xác, liều lượng cụ thể và theo dõi. Từ vườn thương mại đến chậu cây bên cửa sổ.',
      cards: [
        { emoji: '🌾', label: 'Cây trồng đồng ruộng', q: 'Bệnh đốm vòng sớm trên 2 hecta cà chua. Kế hoạch điều trị?' },
        { emoji: '🍒', label: 'Vườn cây ăn quả', q: 'Thối nâu trên cây anh đào của tôi. Liều hữu cơ và hóa học chính xác?' },
        { emoji: '🔬', label: 'Kỹ sư nông nghiệp', q: 'Tính nhu cầu nước (ETc) cho vườn cam của tôi vào tháng 7, Kc 0.85.' },
        { emoji: '🌹', label: 'Khu vườn', q: 'Đốm đen trên hoa hồng của tôi. Trị bằng gì, khi nào?' },
        { emoji: '🌱', label: 'Bãi cỏ', q: 'Các mảng nâu lan trên bãi cỏ của tôi. Đó là gì?' },
        { emoji: '🪴', label: 'Nhà và ban công', q: 'Đốm trắng trên cây húng quế của tôi. Tôi nên làm gì?' },
      ],
    },
    ha: {
      heading: 'Masani guda don duk abin da kake nomawa.',
      sub: 'Ingantaccen gano cuta, daidaitaccen sashi, da biyo baya. Daga gonar kasuwanci zuwa tukunya a kan taga.',
      cards: [
        { emoji: '🌾', label: 'Amfanin gona na fili', q: 'Farin gawayi na farko a hekta 2 na tumatir. Shirin magani?' },
        { emoji: '🍒', label: 'Gonakin itace', q: 'Ruɓewar launin ruwan ƙasa a itatuwan cherry na. Daidaitaccen sashin halitta da na sinadarai?' },
        { emoji: '🔬', label: 'Masana noma', q: 'Lissafa bukatar ruwa (ETc) na gonar lemu na a watan Yuli, Kc 0.85.' },
        { emoji: '🌹', label: 'Gonaki', q: 'Baƙar tabo a furannin fure na. Me ke maganin sa, yaushe?' },
        { emoji: '🌱', label: 'Ciyawa', q: 'Tabo masu launin ruwan ƙasa suna yaɗuwa a ciyawata. Menene shi?' },
        { emoji: '🪴', label: 'Gida da baranda', q: 'Farin tabo a basil na. Me zan yi?' },
      ],
    },
    ur: {
      heading: 'آپ جو کچھ اگاتے ہیں، سب کے لیے ایک ماہر۔',
      sub: 'درست تشخیص، متعین مقدار اور پیروی۔ تجارتی باغ سے لے کر کھڑکی پر رکھے گملے تک۔',
      cards: [
        { emoji: '🌾', label: 'کھیت کی فصلیں', q: '2 ہیکٹر ٹماٹر میں اگیتا جھلساؤ۔ علاج کا منصوبہ؟' },
        { emoji: '🍒', label: 'پھلوں کے باغ', q: 'میرے چیری کے درختوں پر بھورا سڑاؤ۔ درست نامیاتی اور کیمیائی مقدار؟' },
        { emoji: '🔬', label: 'زرعی ماہرین', q: 'جولائی میں میرے سنترے کے باغ کی پانی کی ضرورت (ETc) نکالیں، Kc 0.85۔' },
        { emoji: '🌹', label: 'باغیچے', q: 'میرے گلابوں پر کالا دھبہ۔ اس کا علاج کیا ہے اور کب؟' },
        { emoji: '🌱', label: 'لان', q: 'میرے لان پر بھورے دھبے پھیل رہے ہیں۔ یہ کیا ہے؟' },
        { emoji: '🪴', label: 'گھر اور بالکونی', q: 'میری تلسی پر سفید دھبے۔ میں کیا کروں؟' },
      ],
    },
  };
  return b[lang] ?? b.en;
...
const FEATURES = (lang: string): { icon: LucideIcon; title: string; body: string; accent: boolean }[] => {
  ...
  const f: Record<string, F> = {
    ...
    fr: { t1: 'Pose toutes les questions que tu poserais à un agronome', b1: 'Maladies, irrigation, fertilisation, taille, calendriers de plantation. Si un agronome expérimenté le sait, Oli le sait. Pas seulement une app d\'identification. Ton expert pour tout ce que tu cultives.', t2: 'Fonctionne dans ta langue', b2: '20 langues : grec, anglais, italien, espagnol, français, arabe, turc, hindi, indonésien, bengali, swahili, vietnamien et plus. Conçu pour celles et ceux qui cultivent, partout. Choisis ta langue dans ton profil.', t3: 'Se souvient et fait le suivi', b3: 'Oli se construit avec le temps une mémoire de ce que tu cultives. Fait le suivi comme un vrai agronome pour confirmer que le traitement a fonctionné, et s\'ajuste sinon.' },
    ar: { t1: 'اسأل أي شيء تسأله لمهندس زراعي', b1: 'الأمراض، الري، التسميد، التقليم، مواعيد الزراعة. إذا كان مهندس زراعي خبير يعرفه، فأولي يعرفه. ليس مجرد تطبيق للتعرّف. خبيرك في كل ما تزرعه.', t2: 'يعمل بلغتك', b2: '20 لغة: اليونانية، الإنجليزية، الإيطالية، الإسبانية، الفرنسية، العربية، التركية، الهندية، الإندونيسية، البنغالية، السواحيلية، الفيتنامية والمزيد. مصمم لكل من يزرع، في كل مكان. اختر لغتك من ملفك الشخصي.', t3: 'يتذكّر ويتابع', b3: 'يبني أولي بمرور الوقت ذاكرة لما تزرعه. يتابع كمهندس زراعي حقيقي للتأكد من نجاح العلاج، ويعدّله إن لم ينجح.' },
    tr: { t1: 'Bir ziraat mühendisine soracağın her şeyi sor', b1: 'Hastalıklar, sulama, gübreleme, budama, ekim takvimleri. Deneyimli bir ziraat mühendisi biliyorsa, Oli de bilir. Sadece bir tanıma uygulaması değil. Yetiştirdiğin her şey için uzmanın.', t2: 'Senin dilinde çalışır', b2: '20 dil: Yunanca, İngilizce, İtalyanca, İspanyolca, Fransızca, Arapça, Türkçe, Hintçe, Endonezyaca, Bengalce, Svahili, Vietnamca ve daha fazlası. Her yerde yetiştirenler için yapıldı. Dilini profilinden seç.', t3: 'Hatırlar ve takip eder', b3: 'Oli zamanla ne yetiştirdiğinin hafızasını oluşturur. Gerçek bir ziraat mühendisi gibi tedavinin işe yarayıp yaramadığını takip eder ve gerekirse uyarlar.' },
    ro: { t1: 'Întreabă orice ai întreba un agronom', b1: 'Boli, irigare, fertilizare, tăiere, calendare de semănat. Dacă știe un agronom cu experiență, știe și Oli. Nu doar o aplicație de identificare. Expertul tău pentru tot ce cultivi.', t2: 'Funcționează în limba ta', b2: '20 de limbi: greacă, engleză, italiană, spaniolă, franceză, arabă, turcă, hindi, indoneziană, bengaleză, swahili, vietnameză și altele. Făcut pentru toți cei care cultivă, oriunde. Alege-ți limba din profil.', t3: 'Își amintește și revine', b3: 'Oli își construiește în timp o memorie a ceea ce cultivi. Revine ca un agronom adevărat ca să confirme că tratamentul a funcționat și se ajustează dacă nu.' },
    bg: { t1: 'Попитай каквото би попитал агроном', b1: 'Болести, напояване, торене, резитба, графици за сеитба. Ако го знае опитен агроном, го знае и Oli. Не е просто приложение за разпознаване. Твоят експерт за всичко, което отглеждаш.', t2: 'Работи на твоя език', b2: '20 езика: гръцки, английски, италиански, испански, френски, арабски, турски, хинди, индонезийски, бенгалски, суахили, виетнамски и други. Създадено за всеки, който отглежда, навсякъде. Избери езика си от профила.', t3: 'Помни и проследява', b3: 'Oli изгражда с времето памет за това, което отглеждаш. Проследява като истински агроном дали лечението е подействало и се адаптира, ако не е.' },
    sq: { t1: 'Pyet çfarëdo që do t\'i pyesje një agronomi', b1: 'Sëmundje, ujitje, plehërim, krasitje, kalendarë mbjelljeje. Nëse e di një agronom me përvojë, e di Oli. Jo thjesht një aplikacion njohjeje. Eksperti yt për gjithçka që kultivon.', t2: 'Punon në gjuhën tënde', b2: '20 gjuhë: greqisht, anglisht, italisht, spanjisht, frëngjisht, arabisht, turqisht, hindi, indonezisht, bengalisht, suahili, vietnamisht dhe më shumë. Ndërtuar për këdo që kultivon, kudo. Zgjidh gjuhën nga profili.', t3: 'Kujton dhe ndjek', b3: 'Oli ndërton me kohën një kujtesë të asaj që kultivon. Ndjek si një agronom i vërtetë për të konfirmuar nëse trajtimi funksionoi dhe përshtatet nëse jo.' },
    pt: { t1: 'Pergunta o que perguntarias a um agrónomo', b1: 'Doenças, rega, fertilização, poda, calendários de sementeira. Se um agrónomo experiente sabe, o Oli sabe. Não é só uma app de identificação. O teu especialista para tudo o que cultivas.', t2: 'Funciona na tua língua', b2: '20 línguas: grego, inglês, italiano, espanhol, francês, árabe, turco, hindi, indonésio, bengali, suaíli, vietnamita e mais. Feito para quem cultiva, em qualquer lugar. Define a tua língua no perfil.', t3: 'Lembra-se e faz acompanhamento', b3: 'O Oli constrói ao longo do tempo uma memória do que cultivas. Faz acompanhamento como um agrónomo de verdade para confirmar que o tratamento resultou, e ajusta se não.' },
    de: { t1: 'Frag alles, was du einen Agronomen fragen würdest', b1: 'Krankheiten, Bewässerung, Düngung, Schnitt, Aussaatpläne. Wenn ein erfahrener Agronom es weiß, weiß Oli es auch. Nicht nur eine Erkennungs-App. Dein Experte für alles, was du anbaust.', t2: 'Funktioniert in deiner Sprache', b2: '20 Sprachen: Griechisch, Englisch, Italienisch, Spanisch, Französisch, Arabisch, Türkisch, Hindi, Indonesisch, Bengalisch, Suaheli, Vietnamesisch und mehr. Gemacht für alle, die anbauen, überall. Stell deine Sprache im Profil ein.', t3: 'Merkt sich alles und hakt nach', b3: 'Oli baut mit der Zeit ein Gedächtnis dafür auf, was du anbaust. Hakt wie ein echter Agronom nach, um zu bestätigen, dass die Behandlung gewirkt hat, und passt sie an, falls nicht.' },
    hi: { t1: 'जो किसी कृषि विशेषज्ञ से पूछते, वह पूछें', b1: 'रोग, सिंचाई, खाद, छंटाई, बुवाई का समय। जो एक अनुभवी कृषि विशेषज्ञ जानता है, वह Oli जानता है। यह सिर्फ पहचान करने वाला ऐप नहीं है। आप जो भी उगाते हैं, उसके लिए आपका विशेषज्ञ।', t2: 'आपकी भाषा में काम करता है', b2: '20 भाषाएँ: ग्रीक, अंग्रेज़ी, इतालवी, स्पेनिश, फ़्रेंच, अरबी, तुर्की, हिंदी, इंडोनेशियाई, बंगाली, स्वाहिली, वियतनामी और अन्य। हर जगह उगाने वालों के लिए बनाया गया। अपनी भाषा प्रोफ़ाइल से चुनें।', t3: 'याद रखता है और फ़ॉलो-अप करता है', b3: 'Oli समय के साथ याद रखता है कि आप क्या उगाते हैं। एक असली कृषि विशेषज्ञ की तरह फ़ॉलो-अप करता है कि इलाज काम आया या नहीं, और ज़रूरत हो तो बदलता है।' },
    sw: { t1: 'Uliza chochote ungemuuliza mtaalam wa kilimo', b1: 'Magonjwa, umwagiliaji, urutubishaji, upogoaji, ratiba za kupanda. Kama mtaalam wa kilimo mwenye uzoefu anajua, Oli anajua. Si programu ya kutambua tu. Mtaalam wako kwa chochote unachokuza.', t2: 'Inafanya kazi kwa lugha yako', b2: 'Lugha 20: Kigiriki, Kiingereza, Kiitaliano, Kihispania, Kifaransa, Kiarabu, Kituruki, Kihindi, Kiindonesia, Kibengali, Kiswahili, Kivietinamu na nyingine. Imetengenezwa kwa wanaokuza, kila mahali. Weka lugha yako kutoka wasifu wako.', t3: 'Inakumbuka na kufuatilia', b3: 'Oli hujenga kumbukumbu ya unachokuza kadiri muda unavyopita. Hufuatilia kama mtaalam halisi wa kilimo kuthibitisha kama tiba ilifanya kazi, na hurekebisha kama haikufanya.' },
    bn: { t1: 'একজন কৃষিবিদকে যা জিজ্ঞেস করতেন, তা জিজ্ঞেস করুন', b1: 'রোগ, সেচ, সার, ছাঁটাই, বপনের সময়সূচি। একজন অভিজ্ঞ কৃষিবিদ যা জানেন, Oli তা জানে। এটি শুধু শনাক্তকরণ অ্যাপ নয়। আপনি যা-ই চাষ করেন, তার জন্য আপনার বিশেষজ্ঞ।', t2: 'আপনার ভাষায় কাজ করে', b2: '20টি ভাষা: গ্রিক, ইংরেজি, ইতালিয়, স্প্যানিশ, ফরাসি, আরবি, তুর্কি, হিন্দি, ইন্দোনেশীয়, বাংলা, সোয়াহিলি, ভিয়েতনামি ও আরও। সর্বত্র যারা চাষ করেন তাদের জন্য তৈরি। প্রোফাইল থেকে আপনার ভাষা বেছে নিন।', t3: 'মনে রাখে ও খোঁজ নেয়', b3: 'Oli সময়ের সঙ্গে আপনি কী চাষ করেন তার স্মৃতি তৈরি করে। একজন সত্যিকারের কৃষিবিদের মতো খোঁজ নেয় চিকিৎসা কাজ করেছে কি না, আর না করলে বদলে দেয়।' },
    id: { t1: 'Tanyakan apa pun yang akan Anda tanyakan ke ahli pertanian', b1: 'Penyakit, irigasi, pemupukan, pemangkasan, jadwal tanam. Jika ahli pertanian berpengalaman tahu, Oli tahu. Bukan sekadar aplikasi pengenal. Ahli Anda untuk apa pun yang Anda tanam.', t2: 'Bekerja dalam bahasa Anda', b2: '20 bahasa: Yunani, Inggris, Italia, Spanyol, Prancis, Arab, Turki, Hindi, Indonesia, Bengali, Swahili, Vietnam, dan lainnya. Dibuat untuk siapa pun yang menanam, di mana pun. Atur bahasa Anda dari profil.', t3: 'Mengingat dan menindaklanjuti', b3: 'Oli membangun ingatan tentang apa yang Anda tanam seiring waktu. Menindaklanjuti seperti ahli pertanian sungguhan untuk memastikan perawatan berhasil, dan menyesuaikan jika tidak.' },
    am: { t1: 'ግብርና ባለሙያን የሚጠይቁትን ሁሉ ይጠይቁ', b1: 'በሽታዎች፣ መስኖ፣ ማዳበሪያ፣ መከርከም፣ የመዝሪያ ጊዜ። ልምድ ያለው የግብርና ባለሙያ የሚያውቀውን Oli ያውቃል። ለመለየት ብቻ የሚሆን መተግበሪያ አይደለም። ለሚያበቅሉት ሁሉ ባለሙያዎ ነው።', t2: 'በቋንቋዎ ይሰራል', b2: '20 ቋንቋዎች፦ ግሪክኛ፣ እንግሊዝኛ፣ ጣሊያንኛ፣ ስፓኒሽ፣ ፈረንሳይኛ፣ ዓረብኛ፣ ቱርክኛ፣ ሂንዲ፣ ኢንዶኔዥኛ፣ ቤንጋሊ፣ ስዋሂሊ፣ ቬትናምኛ እና ሌሎችም። በየትኛውም ቦታ ለሚያበቅሉ ሁሉ የተሰራ። ቋንቋዎን ከመገለጫዎ ይምረጡ።', t3: 'ያስታውሳል ይከታተላልም', b3: 'Oli በጊዜ ሂደት የሚያበቅሉትን ያስታውሳል። ሕክምናው እንደሰራ ለማረጋገጥ እንደ እውነተኛ የግብርና ባለሙያ ይከታተላል፣ ካልሰራም ያስተካክላል።' },
    vi: { t1: 'Hỏi bất cứ điều gì bạn sẽ hỏi một kỹ sư nông nghiệp', b1: 'Bệnh hại, tưới tiêu, bón phân, cắt tỉa, lịch gieo trồng. Nếu một kỹ sư nông nghiệp giàu kinh nghiệm biết, Oli biết. Không chỉ là ứng dụng nhận diện. Chuyên gia của bạn cho mọi thứ bạn trồng.', t2: 'Hoạt động bằng ngôn ngữ của bạn', b2: '20 ngôn ngữ: Hy Lạp, Anh, Ý, Tây Ban Nha, Pháp, Ả Rập, Thổ Nhĩ Kỳ, Hindi, Indonesia, Bengali, Swahili, Việt và nhiều hơn nữa. Được tạo cho những người trồng trọt, ở mọi nơi. Chọn ngôn ngữ của bạn trong hồ sơ.', t3: 'Ghi nhớ và theo dõi', b3: 'Oli xây dựng theo thời gian một bộ nhớ về những gì bạn trồng. Theo dõi như một kỹ sư nông nghiệp thực thụ để xác nhận cách điều trị có hiệu quả không, và điều chỉnh nếu không.' },
    ha: { t1: 'Tambayi duk abin da za ka tambayi masanin noma', b1: 'Cututtuka, ban ruwa, takin gargajiya, datsa, jadawalin shuka. Idan gogaggen masanin noma ya sani, Oli ya sani. Ba app na ganewa kawai ba. Masaninka ga duk abin da kake nomawa.', t2: 'Yana aiki da harshenka', b2: 'Harsuna 20: Girkanci, Turanci, Italiyanci, Sifaniyanci, Faransanci, Larabci, Turkanci, Hindi, Indonesiyanci, Bengali, Swahili, Bietnamis da sauransu. An yi shi don duk wanda yake noma, ko\'ina. Zaɓi harshenka daga bayanin martabarka.', t3: 'Yana tunawa kuma yana biyo baya', b3: 'Oli yana gina ƙwaƙwalwar abin da kake nomawa cikin lokaci. Yana biyo baya kamar masanin noma na gaske don tabbatar da maganin ya yi aiki, kuma yana gyara idan bai yi ba.' },
    ur: { t1: 'جو کسی زرعی ماہر سے پوچھتے، وہ پوچھیں', b1: 'بیماریاں، آبپاشی، کھاد، کانٹ چھانٹ، بوائی کے اوقات۔ جو ایک تجربہ کار زرعی ماہر جانتا ہے، وہ Oli جانتا ہے۔ یہ صرف شناخت کرنے والی ایپ نہیں۔ آپ جو کچھ اگاتے ہیں، اس کے لیے آپ کا ماہر۔', t2: 'آپ کی زبان میں کام کرتا ہے', b2: '20 زبانیں: یونانی، انگریزی، اطالوی، ہسپانوی، فرانسیسی، عربی، ترکی، ہندی، انڈونیشی، بنگالی، سواحیلی، ویتنامی اور مزید۔ ہر جگہ اگانے والوں کے لیے بنایا گیا۔ اپنی زبان پروفائل سے منتخب کریں۔', t3: 'یاد رکھتا ہے اور پیروی کرتا ہے', b3: 'Oli وقت کے ساتھ یاد رکھتا ہے کہ آپ کیا اگاتے ہیں۔ ایک حقیقی زرعی ماہر کی طرح پیروی کرتا ہے کہ علاج کارگر ہوا یا نہیں، اور ضرورت ہو تو بدل دیتا ہے۔' },
  };
  const t = f[lang] ?? f['en'];
  return [
...
const TESTIMONIALS = (lang: string) => {
  const map: Record<string, TestimonialItem[]> = {
    el: [
      ...
      { quote: 'Οι πιπεριές μου είχαν κηλίδες που δεν είχα ξαναδεί. Ο Oli ονόμασε την ασθένεια, μου έδωσε τον βιολογικό ψεκασμό και την ακριβή δόση, και μια εβδομάδα μετά επανήλθε να δει το αποτέλεσμα. Οι πιπεριές μου σώθηκαν.', name: 'Αναστασία', crop: 'Λαχανοκαλλιεργήτρια, Βέλο', initial: 'Α' },
      { quote: 'Χρησιμοποιώ τον Oli για να διαχειρίζομαι τον χώρο καλλιέργειας και να βελτιώνω συνεχώς το φυτώριο. Επιπλέον, απαντά στις εκατοντάδες ερωτήσεις φυτών που μου φέρνουν οι πελάτες, με τη θεραπεία και την ακριβή δόση επιτόπου. Σαν να έχω έναν γεωπόνο στην ομάδα.', name: 'Ελένη', crop: 'Φυτώριο, Ξυλόκαστρο', initial: 'Ε' },
    ],
    en: [
      { quote: 'I sent a photo to my agronomist and waited 2 days. Oli gave me a diagnosis in 10 seconds. It was exactly right.', name: 'George', crop: 'Olive grower, Peloponnese', initial: 'G' },
      { quote: 'No more guessing which product to use. It tells me exactly what to buy and at what dose.', name: 'Nikos', crop: 'Vegetable grower, Crete', initial: 'N' },
      { quote: 'I asked when to prune my citrus trees. In seconds I had a specific week-by-week plan. My agronomist charges €80 a visit for exactly that.', name: 'Stavros', crop: 'Citrus grower, Laconia', initial: 'S' },
      ...
      { quote: 'My peppers had spots I had never seen. Oli named the disease, gave me the organic spray and the exact dose, then checked back a week later to see the results. My peppers were saved.', name: 'Anastasia', crop: 'Vegetable grower, Velo', initial: 'A' },
      { quote: 'I use Oli to run my growing area and keep improving the nursery. On top of that, it answers the hundreds of plant questions customers bring me, with the treatment and the exact dose on the spot. Like having an agronomist on the team.', name: 'Eleni', crop: 'Plant nursery, Xylokastro', initial: 'E' },
    ],
    it: [
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#1b1c19] overflow-x-hidden" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {/* ── BREADTH STRIP (range of growers, expert-first) ── */}
      <section className="py-14 bg-[#faf9f4] border-b border-[#f0efea]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#194121] mb-2.5" style={{ fontFamily: "'Noto Serif', serif" }}>
              {breadth.heading}
            </h2>
            <p className="text-sm sm:text-base text-[#606659] max-w-2xl mx-auto leading-relaxed">
              {breadth.sub}
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {breadth.cards.map((c, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl border border-[#e8e8e3] bg-white px-4 py-3.5"
                style={{ boxShadow: '0 1px 6px rgba(25,65,33,0.04)' }}>
                <div className="flex-shrink-0 w-9 h-9 rounded-lg bg-[#f0efea] flex items-center justify-center text-lg leading-none">
                  <span aria-hidden="true">{c.emoji}</span>
                </div>
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-[#4a6b50] mb-1">{c.label}</p>
                  <p className="text-sm text-[#1b1c19] leading-snug">{c.q}</p>
                </div>
              </div>
            ))}
          </div>
