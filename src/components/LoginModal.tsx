  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div
        role="dialog"
        aria-modal="true"
        className="relative w-full max-w-[420px] rounded-[24px] bg-white p-8"
        style={{ boxShadow: '0 8px 40px rgba(25,65,33,0.12)', border: '1px solid rgba(194,201,187,0.2)' }}
      >
        <button
          onClick={onClose}
          className="absolute right-4 top-4 rounded-full p-1.5 text-[#606659] hover:bg-[#f5f4ef] hover:text-[#194121] transition-colors"
          aria-label="Close"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-3">
            <OliLogo size={40} bg="#faf9f4" />
          </div>
          <h2 className="mb-1 text-xl font-bold" style={{ fontFamily: "'Noto Serif', serif", color: '#194121' }}>
            {lang === 'el' ? 'Συνδέσου για να συνεχίσεις' : 'Sign in to continue'}
          </h2>
          <p className="text-sm" style={{ color: '#606659' }}>
            {lang === 'el'
              ? 'Η συνομιλία σου θα αποθηκευτεί στον λογαριασμό σου.'
        ...
        {sent ? (
          <div className="flex flex-col items-center text-center gap-4 animate-fade-in">
            ...
            <div>
              <h3 className="mb-1 text-lg font-semibold" style={{ color: '#194121' }}>{t.checkEmailTitle}</h3>
              <p className="text-sm" style={{ color: '#606659' }}>
                {t.magicLinkSentTo}{' '}
