...
### Fixed
- **VIO follow-up cards now carry context and are tappable**: the in-chat treatment follow-up was a generic, contextless prompt ("Did you apply treatment for your crop?") that surfaced only one reminder at a time and led nowhere on tap. Each follow-up now names the field and crop, the original problem, and how long ago it was logged, puts its question on its own line, and is tappable to jump straight to that field's most recent conversation (falling back to the field page). Multiple due follow-ups now stack instead of hiding all but the oldest. The two duplicated banner blocks were unified into one `VioFollowUpCard` component.
